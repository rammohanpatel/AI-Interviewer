...
const FeedbackPage = () => {
  ...
  useEffect(() => {
    if (!feedback && interview && user?.id && !loading && !isPolling) {
      setIsPolling(true);
      let attempts = 0;
      ...
      const pollInterval = setInterval(async () => {
        ...
        try {
          ...
          if (feedbackResponse.ok) {
            ...
            console.log('Feedback not found yet, polling attempt:', attempts);
          } else {
            console.error('Error fetching feedback:', feedbackResponse.status);
          }
        } catch (error) {
          console.log('Polling for feedback, attempt:', attempts);
        }
        ...
        if (attempts >= maxAttempts) {
          clearInterval(pollInterval);
          setIsPolling(false);
          console.log('Feedback polling timeout');
        }
  ...
  }, [feedback, interview, user?.id, loading, interviewId, isPolling]);
              ...
              <Card className="p-6 bg-card border-border">
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground mb-2">
                    {isPolling ? 'Generating your feedback...' : 'Loading feedback...'}
                  </p>
                  <p className="text-sm text-muted-foreground mb-2">
                    Our AI is analyzing your interview and code. This usually takes 30-60 seconds.
                  </p>
                  {isPolling && pollingAttempts > 0 && (
                    <p className="text-xs text-muted-foreground">
                      Checking... ({pollingAttempts * 2}s elapsed)
                    </p>
                  )}
                  {pollingAttempts >= 60 && !feedback && (
                    <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                      <p className="text-sm text-yellow-600 dark:text-yellow-400">
                        Feedback is taking longer than expected. Please refresh the page in a moment.
                      </p>
                    </div>
                  )}
                </div>
